...
module Authentication =
  ...
  /// source, `u2cp2acOJ`, which can be used to ensure the requested URI and
  /// client parameters match a predetermined whitelist.
  val tryAuthenticateFromSource : u2cp2acOJ:(System.Uri -> ClientParams -> #Job<Auth0Credentials option>) -> uri:System.Uri -> cp:ClientParams -> Alt<AuthenticationResult>
...
module Client =
  ...
  val addAuth0TokenHeaderFromSource : u2cp2atOJ:(System.Uri -> ClientParams -> #Job<Auth0Token option>) -> cp:ClientParams -> req:Request -> Alt<Request>
  ...
  val tryGetResponseWithAuthSource : u2cp2acOJ:(System.Uri -> ClientParams -> #Job<Auth0Token option>) -> cp:ClientParams -> req:Request -> Alt<Choice<Response,exn>>
  ...
  val tryWithRetryOnAuthRequired : u2cp2atOJ:(System.Uri -> ClientParams -> #Job<Auth0Token option>) -> req:Request -> respCJ:#Job<Choice<Response,exn>> -> Alt<Choice<Response,exn>>
  ...
  val tryGetResponseWithRetryOnAuthRequired : u2cp2acOJ:(System.Uri -> ClientParams -> #Job<Auth0Token option>) -> req:Request -> Alt<Choice<Response,exn>>
...
module CacheExpirationParameters =
  ...
  val create : getNow:Job<int64> -> defMaxAge:int64 -> toMaxAge:int64 -> failMaxAge:int64 -> CacheExpirationParameters
  /// Creates a `CacheExpirationParameters` value which specifies the default max age
  /// in seconds to use for any token which doesn't specify its own expiration as
  /// well as the how long to cache timeouts and API failures.
  val create' : defMaxAge:int64 -> toMaxAge:int64 -> failMaxAge:int64 -> CacheExpirationParameters
...
module TokenCache =
  ...
  val putToken : tc:TokenCache -> cp:ClientParams -> a0token:Auth0Token -> exp:int64 -> Alt<unit>
  ...
  val putAuthenticationResult : cep:CacheExpirationParameters -> tc:TokenCache -> cp:ClientParams -> ar:Authentication.AuthenticationResult -> Alt<unit>
  /// Attempts to get an Auth0 authentication result from the cache. Does not check the expiration of the returned value.
  val tryGetEntry : tc:TokenCache -> cp:ClientParams -> Alt<Choice<Auth0Token,Auth0ApiFailure> option>
  /// Attempts to get an unexpired Auth0 authentication token from the cache.
  val tryGetToken : cep:CacheExpirationParameters -> tc:TokenCache -> cp:ClientParams -> Alt<Auth0Token option>
  ...
  val tryGetTokenWithFill : cep:CacheExpirationParameters -> cp2arJ:(ClientParams -> #Job<Authentication.AuthenticationResult>) -> tc:TokenCache -> cp:ClientParams -> Alt<Auth0Token option>
